...
#define enablePin  48   // Connects to the RFID's ENABLE pin
...
int ledPin = 9; //YELLOW LED INDICATES RFID DETECTION
...
void loop()  // Main code, to run repeatedly
{
  ...
  */   
  ...
  while(1)
  {
    if (Serial3.available() > 0) // If there are any bytes available to read, then the RFID Reader has probably seen a valid tag
    {
      rfidData[offset] = Serial3.read();  // Get the byte and store it in our buffer
